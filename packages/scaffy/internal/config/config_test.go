...
func TestEnsureDefaultConfig_AlreadyExists(t *testing.T) { //nolint:paralleltest // t.Setenv used
	viper.Reset()
	tmpHome := t.TempDir()
...
func TestEnvVariableOverride(t *testing.T) { //nolint:paralleltest // t.Setenv used
	viper.Reset()
	tmpDir := t.TempDir()

	t.Setenv("UP_SKILL_AUTHOR", "env-author")
	t.Setenv("UP_SKILL_LOG_LEVEL", "warn")
...
func TestEnvVariableOverride_OutputDir(t *testing.T) { //nolint:paralleltest // t.Setenv used
	viper.Reset()
	tmpDir := t.TempDir()

	t.Setenv("UP_SKILL_OUTPUT_DIR", "/env/output")
	t.Setenv("UP_SKILL_TEMPLATES_DIR", "/env/templates")
